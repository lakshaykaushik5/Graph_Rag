...
from config import EMBEDDING_CHAT_MODEL
import os
...
def add_data(collection_name,chunks_data):
    try:
        ...
        check_collection_exist = collection_name in collection_names

        print(check_collection_exist)
        ...
        QdrantVectorStore.add_documents(
            ...
            collection_name = collection_name,
            embedding = OpenAIEmbeddings(model=EMBEDDING_CHAT_MODEL,api_key=os.getenv('OPENAI_KEY')),
            client = client
        )
...
def get_all_collections():
    try:
        ...
        print(f"Error in avaliable_collections() :::: {err}")
        return []